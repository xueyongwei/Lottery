...
class APICenter: NSObject {
    ...
    static var currentEnvironment: Environment{
        let stateInt = UserDefaultsCenter.DeveloperAPIEnvironment.int
    ...
    fileprivate static func base_URL() -> String {
        ...
        case .develop:
            return "http://123.59.124.153/api"
        case .gray:
    ...
    /// 得到一个url，如果不能得到，会崩溃以便debug
    static func urlFromString(_ urlstr:String) ->URL{
...
extension APICenter{
    ...
    private static func Base_Lottery_URL() -> String {
        return base_URL() + "/lottery"
    }
    
    struct Lottery {
        /// 首页接口
        static func index() ->URL{
            let urlstr = APICenter.Base_Lottery_URL() + "/index"
            return APICenter.urlFromString(urlstr)
        }
        /// 获取主彩种
        static func getLotterycCategory()->URL{
            let urlstr = APICenter.Base_Lottery_URL() + "/get/lottery/category"
            return APICenter.urlFromString(urlstr)
        }
